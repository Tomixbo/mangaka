    ...
    if request.method == 'POST':
        ...
        for image_path in sorted(image_paths):
            ...
            with open(full_image_path, 'rb') as img_file:
                response = requests.post("http://127.0.0.1:5000/detect_panels_texts", files={"file": img_file})
            if response.status_code != 200:
                print(f"Detection API failed for {image_path}: {response.json()}.")
                continue
            ...
            for panel_id, texts_in_panel in panel_texts.items():
                ...
                for text_box in sorted_texts:
                    ...
                    if preprocessed_text is not None:
                        ...
                        response = requests.post("http://127.0.0.1:5000/text_recognition", files={"file": io.BytesIO(buffer)})
                        if response.status_code == 200:
                ...
                page_index = image_paths.index(image_path)
                ...
                if processed_texts:
                    ...
                    response = requests.post("http://127.0.0.1:5000/text_to_speech", data=tts_payload)
